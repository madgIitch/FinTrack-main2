...
messaging.onBackgroundMessage((payload)=>{
    ...
    self.registration.showNotification(title || "Notificaci\xf3n", {
        body: body || '',
...
async function doFullSync() {
    ...
    try {
        ...
        } else console.warn('[SW] No se pudo guardar el resumen diario');
    } catch (err) {
...
function storeInIndexedDB(key, value) {
    console.log(`[SW] storeInIndexedDB \u{2192} guardando clave: ${key}`, value);
    return new Promise((resolve, reject)=>{
        ...
        request.onsuccess = ()=>{
            ...
            putRequest.onsuccess = ()=>{
                console.log(`[SW] storeInIndexedDB \u{2192} OK: ${key}`);
                resolve();
            };
            putRequest.onerror = (e)=>{
                console.error(`[SW] storeInIndexedDB \u{2192} ERROR put: ${e}`);
                reject(putRequest.error);
            };
            tx.oncomplete = ()=>{
                db.close();
                console.log("[SW] storeInIndexedDB \u2192 transacci\xf3n cerrada");
            };
            tx.onerror = (e)=>{
