...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    (0, _auth.onAuthStateChanged)((0, _firebaseJs.auth), async (user)=>{
        ...
        btnSaveSettings.addEventListener('click', async ()=>{
            ...
            try {
                ...
                btnSaveSettings.textContent = "\xa1Guardado!";
                setTimeout(()=>{
     ...
     */ async function fetchGroups() {
            ...
            try {
                const groupsSnap = await (0, _firestore.getDocs)((0, _firestore.collection)(db, 'groups'));
                allGroups = [];
     ...
     */ async function fetchBudgetsForUser(userId) {
            ...
            budgetsMap = {};
            try {
                const budgetsQuery = (0, _firestore.query)((0, _firestore.collection)(db, 'budgets'), (0, _firestore.where)('userId', '==', userId));
                const budSnap = await (0, _firestore.getDocs)(budgetsQuery);
     ...
     */ function showAddBudgetRow() {
            ...
            if (document.querySelector('.adding-budget-row')) {
                console.log("[settings.js] Ya existe un row de a\xf1adir abierto");
                return;
            }
            ...
            saveBtn.addEventListener('click', async ()=>{
                ...
                if (!selectedGroupId) {
                    alert("Selecciona un grupo v\xe1lido.");
                    return;
                }
                const value = parseFloat(input.value);
                if (isNaN(value) || value < 0) {
                    alert("Ingresa un monto v\xe1lido (\u2265 0).");
                    return;
                }
                const now = new Date();
                const periodStart = new Date(now.getFullYear(), now.getMonth(), 1).toISOString();
                const budgetDocId = `${uid}_${selectedGroupId}`;
                const budgetRef = (0, _firestore.doc)(db, 'budgets', budgetDocId);
                const budgetData = {
                    userId: uid,
                ...
                try {
                    await (0, _firestore.setDoc)(budgetRef, budgetData, {
                        merge: true
                    });
     ...
     */ function createBudgetRow({ groupId, groupName, currentLimit, isExisting }) {
            console.log(`[settings.js] createBudgetRow \u{2192} ${groupId}, existente:${isExisting}`);
            const row = document.createElement('div');
            ...
            editBtn.addEventListener('click', async ()=>{
                if (!isEditing) {
                    ...
                    try {
                        await (0, _firestore.setDoc)(budgetRef, updatedData, {
                            merge: true
                        });
                        budgetsMap[groupId] = {
                            monthlyLimit: newValue,
            ...
            deleteBtn.addEventListener('click', async ()=>{
                if (!confirm(`\xbfEliminar presupuesto para "${groupName}"?`)) return;
                const budgetDocId = `${uid}_${groupId}`;
                const budgetRef = (0, _firestore.doc)(db, 'budgets', budgetDocId);
                try {
                    await (0, _firestore.deleteDoc)(budgetRef);
            ...
            console.log(`[settings.js] createBudgetRow \u{2192} fila creada para ${groupId}`);
        }
    });
});
