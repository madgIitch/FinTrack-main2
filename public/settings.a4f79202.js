...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    (0, _auth.onAuthStateChanged)((0, _firebaseJs.auth), async (user)=>{
        ...
        try {
            ...
            if (snap.exists()) loadedSettings = snap.data().settings || {};
        ...
        if (!Object.keys(budgetsMap).length) budgetsContainer.appendChild(createBudgetRow());
    });
    btnSave.addEventListener('click', async ()=>{
        ...
        rows.forEach((row, idx)=>{
            ...
            if (seen.has(cat)) {
                valid = false;
                errorMessage = `La categor\xeda "${cat}" est\xe1 repetida.`;
                return;
            }
            seen.add(cat);
