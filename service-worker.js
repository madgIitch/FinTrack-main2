...
async function doFullSync() {
  ...
  try {
    ...
    console.log('[SW] Sincronización de balance omitida (la gestiona home.js)');

    // ── Paso 5: Guardar resumen diario del mes ────────────────────────
    const month = new Date().toISOString().slice(0, 7); // 'YYYY-MM'
    console.log('[SW] Enviando petición a get_daily_summary...');
    const totalsRes = await fetch(`${apiUrl}/plaid/get_daily_summary`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ userId: uid, month })
    });
