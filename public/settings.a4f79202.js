...
async function subscribeUserToPush() {
    ...
    try {
        ...
        const fcmToken = await (0, _messaging.getToken)(messaging, {
            vapidKey: VAPID_PUBLIC_KEY
        });
        console.log('[settings.js] FCM token:', fcmToken);
        // 6) Enviar todo al backend
        const userId = (0, _firebaseJs.auth).currentUser.uid;
        console.log("[settings.js] Enviando suscripci\xf3n al backend para userId:", userId);
        ...
        console.log("[settings.js] Suscripci\xf3n guardada en servidor");
        console.groupEnd();
        return sub;
    } catch (err) {
...
async function unsubscribeUserFromPush() {
    ...
    const userId = (0, _firebaseJs.auth).currentUser.uid;
    console.log("[settings.js] Notificando al backend para eliminar suscripci\xf3n:", userId);
    ...
    console.log("[settings.js] Suscripci\xf3n eliminada en servidor");
}
...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    btnSave.addEventListener('click', async ()=>{
        ...
        rows.forEach((row, i)=>{
            ...
            if (isNaN(amt) || amt < 0) {
                valid = false;
                errorMsg = `Fila ${i + 1}: monto inv\xe1lido para "${cat}".`;
                return;
            }
            if (seen.has(cat)) {
                valid = false;
